...
public class BezierCurve
{
    ...
    public static Vector3 Point3(float t, List<Vector3> ctrlPts)
    {
        int N = ctrlPts.Count - 1;
        if (N > 16)
        {
            Debug.Log("You have used more than 16 control points. The maximum control points allowed is 16.");
            ctrlPts.RemoveRange(16, ctrlPts.Count - 16);
        }
        if (t <= 0) return ctrlPts[0];
        if (t >= 1) return ctrlPts[ctrlPts.Count - 1];

        Vector3 p = new Vector3();

        for (int i = 0; i < ctrlPts.Count; ++i)
        {
            Vector3 bn = Bernstein(N, i, t) * ctrlPts[i];
            p += bn;
        }
